...
export async function handleLocationsList(ctx: BotContext) {
    ...
    if (!allLocations.length)
        return editOrSend(ctx, "No locations found.");
    ...
    kb.row().url("\uD83D\uDDFA Interactive Map", "https://www.google.com/maps/search/AAU+4-Killo+Evangelical+Christian+Students+Fellowship");
...
export async function handleLocationDetail(ctx: BotContext, id: string) {
    try {
        ...
        const hasCoords = loc.coordinates && loc.coordinates.latitude && loc.coordinates.longitude;
        if (hasCoords) {
            await deleteLastBotMessage(ctx);
            const msg = await ctx.replyWithVenue(loc.coordinates.latitude, loc.coordinates.longitude, loc.name, loc.address || "Addis Ababa");
