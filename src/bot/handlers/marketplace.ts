...
export async function handleProductsList(ctx: BotContext) {
  ...
  const allProducts = Array.isArray(result) ? result : result.products || result.data || [];
  
  if (!allProducts.length) return editOrSend(ctx, "No products found in marketplace.");
  ...
  const kb = buildPaginationKeyboard("marketplace", page, hasMore, "fi_menu");
...
export async function handleProductDetail(ctx: BotContext, id: string) {
  try {
    ...
    const kb = new InlineKeyboard()
      .text("Order Now", `product_order_${p.id}`).row()
