...
export async function completePayment(ctx: BotContext, text: string) {
    ...
    try {
        ...
        if (checkoutUrl) {
            ...
            await ctx.reply(`Giving initialized for ${amount} ETB. Complete it via the link below:`, {
                reply_markup: kb,
            });
        }
        else {
            await ctx.reply("Giving initialized. Follow the instructions on your phone.");
        }
    }
    catch (err: any) {
