...
import { sharePhoneKeyboard, mainReplyKeyboard, selectionKeyboard, TEAM_NAMES, DEPARTMENTS, YEARS, } from "../keyboards";
...
async function sendFellowInfoPhoto(ctx: BotContext) {
    ...
    const caption = `AAU 4-Killo Evangelical Christian Students' Fellowship (ECSF) official telegram bot.\n\n` +
    ...
    const kb = new InlineKeyboard().url("Official Channel", "https://t.me/AAU_4Killo_Fellowship");
...
export async function handleStart(ctx: BotContext) {
    ...
    await ctx.reply("Welcome to 4killo ECSF Bot. Share your phone number to sign in or get started.", {
...
export async function handleContact(ctx: BotContext) {
    ...
    try {
        ...
        await ctx.reply(`Account found for ${user.fullName}. Please enter your password to sign in.`, {
        ...
        await deleteLastBotMessage(ctx);
        await ctx.reply("Welcome. Lets create your profile. What is your full name?", {
            reply_markup: { remove_keyboard: true },
        });
    }
}
export async function handlePasswordSubmitted(ctx: BotContext, password: string) {
...
export async function handleYearCollected(ctx: BotContext, year: string) {
    ...
    await ctx.reply("Please enter a password for your account (at least 6 characters). You'll need this to sign in later.", {
        reply_markup: { remove_keyboard: true },
    });
}
export async function handlePasswordCollected(ctx: BotContext, password: string) {
    ...
    await ctx.reply("Almost done! Would you like to upload a profile picture? Send it as a photo or click skip.", {
        reply_markup: kb,
    });
}
