...
import { handleStart, handleContact, handleNameCollected, handleTeamCollected, handleDepartmentCollected, handleYearCollected, handlePasswordCollected, finalizeRegistration, handlePasswordSubmitted, handleLogout, } from "./bot/handlers/auth";
import { handleHome } from "./bot/handlers/home";
import { handleFellowInfo, handleFellowFeatures, handleSocialLinks, } from "./bot/handlers/fellow-info";
import { handleEventsList, handleEventRegister } from "./bot/handlers/events";
import { handleDevotionsList, handleDevotionDetail, } from "./bot/handlers/devotions";
import { handleTeamsList, handleTeamDetail, handleTeamJoin, } from "./bot/handlers/teams";
import { handleLocationsList, handleLocationDetail, } from "./bot/handlers/locations";
import { handleProgramsList, handleProgramDetail, } from "./bot/handlers/programs";
import { handleLeadersList } from "./bot/handlers/leaders";
import { handleProductsList, handleProductDetail, } from "./bot/handlers/marketplace";
import { handleMyProfile, handleJoinRequests, handleMyOrders, handleMyGivings, } from "./bot/handlers/profile";
import { handlePayments, handleDonateStart, completePayment, } from "./bot/handlers/payments";
...
bot.on("callback_query:data", async (ctx) => {
    ...
    try {
        ...
        if (data === "skip_photo")
            return finalizeRegistration(ctx);
        ...
        if (data === "profile_join_requests")
            return handleJoinRequests(ctx);
        if (data === "profile_orders")
            return handleMyOrders(ctx);
        if (data === "profile_givings")
            return handleMyGivings(ctx);
        if (data === "pay_donate")
            return handleDonateStart(ctx);
        if (data === "pay_history")
            return handleMyGivings(ctx);
